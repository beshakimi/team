...
export default function AllProjects() {
    ...
    return (
            ...
            <div className='grid grid-cols-2 lg:grid-cols-3 md:gap-6 gap-4 mt-4 md:mt-6'>

                {/* projectts */}
                {Portfolioes.map(project => (
                    <div key={project.id} className='border border-gray-400 flex flex-col gap-2'>
                        <img src={`${imageUrl}${project.image}`} alt="Besmillah Hakimi" className='w-full h-32 md:44 lg:h-60 object-cover' />

                        <div className='flex flex-col gap-2 px-4 pb-4 justify-center'>

                            <h1 className=' text-gray-800 text-sm sm:text-base font-semibold w-full text-center'>{project.title}</h1>
                            <p className='text-sm sm:text-sm text-gray-500 w-full md:text-center'>{project.description?.slice(0, 70)} ...</p>
