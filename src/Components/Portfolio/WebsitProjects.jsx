...
export default function WebsiteProjects() {
    ...
    useEffect(() => {
        axios.get(Url)
            .then((res) => {
                const websiteProjects = res.data.filter(project => project.categories.includes('Website'));
                setPortfolio(websiteProjects);
            })
                ...
                {Portfolioes.map(project => (
                    <div key={project.id} className='border border-gray-400 flex flex-col gap-2'>
                        ...
                        <div className='flex flex-col gap-2 px-4 pb-4 justify-center'>
                            ...
                            <p className='text-sm sm:text-sm text-gray-500 w-full text-center'>{project.description?.slice(0, 70)} ...</p>
