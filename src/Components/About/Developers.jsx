                ...
                {teamData.map(developer => (
                    <Link onClick={() => window.scrollTo(0, 0)} key={developer.id} to={`/developer/${developer.id}`}>
                        <div className='rounded-md bg-[#ffe0cc] p-2 lg:p-5 flex flex-col gap-1 md:gap-2 hover:cursor-pointer hover:shadow-lg hover:shadow-orange-300 ease-in duration-150'>
                            <img src={`${mediaServerHostname}${developer.profile_image}`} alt={`${developer.firstname} ${developer.lastname}`} className='w-full h-32 md:h-44 lg:h-60 object-cover rounded-md' />
                            ...
                            <p className='text-xs md:text-sm text-gray-500 lg:text-justify'>{developer.bio?.slice(0, 100)} ...</p>
