...
import blogPosts from '../../data/blogPosts'; // مسیر را با پروژه‌ات تنظیم کن

export default function RelatedPost() {
    const navigate = useNavigate();
    const { id } = useParams();
    const imageUrl = '/images/'; // مسیر تصاویر محلی

    // فیلتر کردن پست‌های مرتبط (همه به جز همان پست فعلی)
    ...
    return (
        <div className='w-full flex flex-col gap-4 my-10'>
            ...
            <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 md:gap-6'>
                {relatedPosts.map((post) => (
                    <div key={post.id} onClick={() => handleLinkClick(post.id)} className='flex flex-col gap-1 w-full bg-orange-100 p-2 md:p-4 rounded-sm hover:cursor-pointer hover:text-orange-500 ease-in duration-150'>
                        ...
                        <div className='w-full h-28 relative'>
                            <img src={`${imageUrl}${post.image}`} alt={post.title} className='w-full h-full object-cover hover:scale-105 ease-in duration-200' />
